...
export function Formulario() {
  ...
  const fetchDonadores = () => {
    fetch('https://quenotadefuturo-backend.herokuapp.com/api/donadores')
      .then((res) => res.json())
      .then(setDonadores);
  };
  ...
  const deleteDonador = (id) => {
    if (window.confirm('Realmente desea eliminar el Donador?')) {
      fetch(`https://quenotadefuturo-backend.herokuapp.com/api/donadores/${id}`, {
        method: 'DELETE',
        headers: {
  ...
  const agregarDonador = (e) => {
    e.preventDefault();
    if (state._id) {
      fetch(`https://quenotadefuturo-backend.herokuapp.com/api/donadores/${state._id}`, {
        method: 'PUT',
