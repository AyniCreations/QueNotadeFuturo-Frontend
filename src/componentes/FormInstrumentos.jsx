...
export function FormInstrumentos() {
  ...
  const fetchInstrumentos = () => {
    fetch('https://quenotadefuturo-backend.herokuapp.com/api/instrumentos')
      .then((res) => res.json())
  ...
  const deleteInstrumento = (id) => {
    if (window.confirm('Realmente desea eliminar el Instrumento?')) {
      fetch(`https://quenotadefuturo-backend.herokuapp.com/api/instrumentos/${id}`, {
        method: 'DELETE',
        headers: {
  ...
  const agregarInstrumento = (e) => {
    e.preventDefault();
    if (state._id) {
      fetch(`https://quenotadefuturo-backend.herokuapp.com/api/instrumentos/${state._id}`, {
        method: 'PUT',
